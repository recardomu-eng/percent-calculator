    ...
    if symbol == "S&P 500":
        ...
        if st.button("Посчитать лот для S&P 500", key="sp_btn"):
            if stop > 0:
                ...
                # Цветовая индикация в зависимости от процента риска
                if risk_percent > 3.0:
                    st.error(f"⚠️ ВНИМАНИЕ: Слишком высокий риск ({risk_percent}%)!")
                    st.warning("Рекомендуемый риск для сохранения депозита: 1-2%.")
                elif risk_percent >= 1.0:
                    st.success(f"✅ Риск в норме ({risk_percent}%). Хороший план!")
                else:
                    st.info(f"🛡️ Консервативный риск ({risk_percent}%). Очень безопасно.")
